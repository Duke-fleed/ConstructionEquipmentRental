...
export default class RentalOrderComponent extends React.Component {
  ...
  submitRentalRequest = () => {
    ...
    if (selectedItems.length === 0)
      ...
      if (!this.areAllItemDaysEntered(selectedItems, selectedIdsWithDays)) {
        ...
        this.setState({ displayAlert: false, alertText: null, openAlertDialog: true });
      }
    }
  }
  ...
  render() {
    const { error, isLoaded, items, selectedItems, selectedDays, openDialog, lastOrderId, displayAlert, alertText, openAlertDialog } = this.state;
    if (error) {
      ...
      return (
        <div>
          ...
          <AlertDialog 
            ...
            alertText={"Are you sure you want request this item(s)?"} 
            onClose={() => this.setState({ openAlertDialog: false })} 
            onAgree={this.onAlertDialogConfirm} />
        </div>
      );
    }
  }
